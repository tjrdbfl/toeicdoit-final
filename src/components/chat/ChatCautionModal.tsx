...
const ChatCautionModal = ({ type, chat, option, setOpen, sender, name, profile, admin, userData }: {
    ...
    const handleUserInfo = async () => {
        ...
        if (response.message === 'SUCCESS' && response.data!==undefined) {
            setUserId(Number(response.data));
        }
    }
                            ...
                            <div
                                ...
                                <div className="flex flex-row gap-x-4 items-center">
                                    <Image loading="lazy"
                                        ...
                                        height={40}
                                    />
                                    {admin!==undefined && item.userId===admin[0] ?
                                      <Image loading="lazy"
                                      src={"/svgs/icons/star-icon.svg"}
                                      alt={"chat-main"}
                                      className="h-[20px] w-[20px] rounded-full bg-pink-500"
                                      ...
                                      height={20}
                                    />:
                                    item.userId === userId.toString() && <div className="bg-zinc-700 text-white p-1 rounded-lg text-[10px] h-[23px] w-[23px] text-center">나</div>}  
