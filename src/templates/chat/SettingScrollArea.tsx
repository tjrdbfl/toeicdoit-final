...
const SettingScrollArea = () => {
    ...
    const handleUserInfo = async () => {
        ...
        if (response.message === 'SUCCESS') {
            setUserId(response.data === undefined ? '' : response.data);
        } else {
            setUserId('');
        }
    }
        ...
        <ScrollArea className="h-[300px] overflow-y-auto p-3">
            <div
                ...
                {data?.pages.map((page, index) => {
                    ...
                    return (<div
                        key={index}>
                        {page.data?.filter((chat) =>
                            Array.isArray(chat.adminIds) && chat.adminIds.includes(userId)).map((item, index) => {

                                console.log(JSON.stringify(item.adminIds));
                                return (
                                    <div
                                        onClick={() => {
